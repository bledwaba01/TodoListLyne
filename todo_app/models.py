# todo_app/models.py - UPDATED VERSION WITH SOFT DELETE
...
class Habit(models.Model):
    ...
    # NEW: Property to check if completed today
    @property
    def is_completed_today(self):
        """Check if habit was completed today"""
        today = timezone.now().date()
        return self.logs.filter(log_date=today, is_completed=True).exists()
    ...
    def last_7_days(self):
        ...
        for i in range(6, -1, -1):
            ...
            completed = self.logs.filter(log_date=date_check, is_completed=True).exists()
    ...
    # NEW: Soft Delete Methods
    def soft_delete(self):
        """Mark habit as deleted instead of removing from database"""
        self.is_deleted = True
        self.deleted_at = timezone.now()
        self.save()
