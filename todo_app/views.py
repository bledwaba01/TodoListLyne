...
def edit_task(request, task_id):
 ...
 #===== UPDATE YOUR EXISTING DELETE VIEWS =====

@login_required
def delete_task(request, task_id):
    """Soft delete a task (move to recycle bin)"""
    if request.method == 'POST':
        task = get_object_or_404(Task, id=task_id, user=request.user)
        task.soft_delete()
...
@login_required
def delete_habit(request, habit_id):
    """Soft delete a habit (move to recycle bin)"""
    if request.method == 'POST':
        habit = get_object_or_404(Habit, id=habit_id, user=request.user)
        habit.soft_delete()
        messages.success(request, 'Habit moved to recycle bin.')
    return redirect('habits_view')
...
def recycle_bin_view(request):
    """Display all deleted tasks and habits"""
    deleted_tasks = Task.objects.filter(user=request.user, is_deleted=True).order_by('-deleted_at')
    deleted_habits = Habit.objects.filter(user=request.user, is_deleted=True).order_by('-deleted_at')
    
    # Get user profile for theme
    profile = request.user.profile if hasattr(request.user, 'profile') else None
    
    context = {
        'deleted_tasks': deleted_tasks,
        'deleted_habits': deleted_habits,
        'tasks_count': deleted_tasks.count(),
        'habits_count': deleted_habits.count(),
        'profile': profile,
        'user_name': profile.custom_name if profile else request.user.username,
    }
    return render(request, 'todo_app/recycle_bin.html', context)
...
def restore_habit(request, habit_id):
    ...
    if request.method == 'POST':
        habit = get_object_or_404(Habit, id=habit_id, user=request.user, is_deleted=True)
        habit.restore()
        messages.success(request, f'Habit "{habit.title}" restored successfully!')
    return redirect('recycle_bin')


# ===== PERMANENT DELETE VIEWS =====
...
def permanent_delete_task(request, task_id):
    ...
    if request.method == 'POST':
        ...
        task.delete()  # Actually delete from database
        messages.success(request, f'Task "{task_title}" permanently deleted.')
    return redirect('recycle_bin')


@login_required
def permanent_delete_habit(request, habit_id):
    """Permanently delete a habit from recycle bin"""
    if request.method == 'POST':
        habit = get_object_or_404(Habit, id=habit_id, user=request.user, is_deleted=True)
        habit_title = habit.title
        habit.delete()  # Actually delete from database
        messages.success(request, f'Habit "{habit_title}" permanently deleted.')
    return redirect('recycle_bin')
...
def empty_recycle_bin(request):
    ...
    if request.method == 'POST':
        tasks_count = Task.objects.filter(user=request.user, is_deleted=True).count()
        habits_count = Habit.objects.filter(user=request.user, is_deleted=True).count()
        
        Task.objects.filter(user=request.user, is_deleted=True).delete()
        Habit.objects.filter(user=request.user, is_deleted=True).delete()
        
        total = tasks_count + habits_count
        messages.success(request, f'Recycle bin emptied. {total} items permanently deleted.')
...
def edit_habit(request, habit_id):
    ...
    habit = get_object_or_404(Habit, id=habit_id, user=request.user, is_deleted=False)
    
    if request.method == 'POST':
        habit.title = request.POST.get('title', '').strip()
        habit.goal_description = request.POST.get('goal_description', '').strip()
        habit.frequency = request.POST.get('frequency', 'daily')
        
        if not habit.title:
            messages.error(request, 'Habit name cannot be empty.')
            return redirect('habits_view')
        
        if not habit.goal_description:
...
# Make sure your dashboard view filters out deleted tasks

@login_required
def dashboard_view(request):
    """Main dashboard view - UPDATED to exclude deleted tasks"""
    profile = request.user.profile if hasattr(request.user, 'profile') else None
    
    # Filter out deleted tasks
    all_tasks = Task.objects.filter(user=request.user, is_deleted=False)
    active_tasks = all_tasks.filter(is_completed=False)
    completed_tasks = all_tasks.filter(is_completed=True)
    
    # Get tasks needing attention (overdue or high priority)
    attention_tasks = active_tasks.filter(
        models.Q(due_date__lt=timezone.now().date()) |  # Overdue
        models.Q(priority=1)  # High priority
    ).distinct()
...
# ===== UPDATE YOUR EXISTING HABITS VIEW =====
# Make sure your habits view filters out deleted habits

@login_required
def habits_view(request):
    """Habits tracking view - UPDATED to exclude deleted habits"""
    profile = request.user.profile if hasattr(request.user, 'profile') else None
    
    # Filter out deleted habits
    user_habits = Habit.objects.filter(user=request.user, is_deleted=False)
    
    # Calculate statistics
    total_habits = user_habits.count()
    habits_completed_today = [h for h in user_habits if h.is_completed_today]
    total_streak = sum(h.get_streak() for h in user_habits)
    
    # Calculate completion rate
    if total_habits > 0:
        completion_rate = int((len(habits_completed_today) / total_habits) * 100)
    else:
        completion_rate = 0
